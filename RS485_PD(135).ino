...
void loop() {
  SerialUSB.print("Reading Vial:");   //escreve 'reading vial'
  SerialUSB.println(active_vial);   //indica qual o vial em análise
  read_MuxShield();;      //invoca a subrrotina readMuxShield
  if (stringComplete) {   //se a string estiver completa
    SerialUSB.println(inputString);    //escreve a inputString no monitor serial
    in.analyzeAndCheck(inputString);   //percorre e verifica a inputstring. 

    // Photodiode Logic
    if (in.addressFound) {   //após verificada a inputstring, se o endereço estiver presente: 
      if (in.input_array[0] == "i" || in.input_array[0] == "r") {  //veriifica se o comando é imediato ou recorrente
        
        SerialUSB.println("Saving PD Setting");
        saved_PDaveraged = in.input_array[1].toInt();   //salva o 2o elemento do inputarray (o primeiro é o endereço)
        
        SerialUSB.println("Echoing New PD Command");
        new_PDinput = true;  //indica que há novo PDinput
        dataResponse(); //invoca subrrotina dataResponse

        SerialUSB.println("Waiting for OK to execute...");
      }
      if (in.input_array[0] == "a" && new_PDinput) { //se há novo input e o comando for de acknowledgement to run
        PDtimes_averaged = saved_PDaveraged;    //salva os valores de PD
        SerialUSB.println("PD Command Executed!");
        new_PDinput = false;  //retorna o estado de input de PD para falso.
      }        
